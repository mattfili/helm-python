...
Use the real GitHub REST API through helm's `openapi()` factory. A curated 18-endpoint OpenAPI spec gives you typed access to users, repos, issues, and search — all auto-discovered via `search()`.
...
The curated spec at `../shared/github_spec.json` contains 18 GET-only endpoints from the GitHub API. The `openapi()` factory turns each into a helm operation:
