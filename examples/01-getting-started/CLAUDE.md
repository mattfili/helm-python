...
Foundation pattern for all helm usage: create instance, register skills, call typed operations, discover via search.
...
## Key Imports
```python
from helm import Helm, HelmOptions, fs, git, grep, edit, http, shell
```

## How to Replicate This Pattern
1. Import `Helm`, `HelmOptions`, and desired skill factories
2. Create `Helm(HelmOptions(default_permission="allow"))` for scripts (no interactive prompts)
