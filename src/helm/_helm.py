...
def _make_bound(
    ...
    async def _wrapped(*args: Any, **kwargs: Any) -> Any:
        permission = resolve_permission(
            qn, op_default, helm_self._policy, helm_self._global_default
        )
        ...
        if permission == "ask":
            if helm_self._on_permission_request is None:
                raise PermissionDeniedError(qn)

            result = helm_self._on_permission_request(qn, list(args))
...
class Helm:
    def __init__(self, options: HelmOptions | None = None) -> None:
        opts = options or HelmOptions()
...
def create_helm(options: HelmOptions | None = None) -> Helm:
    return Helm(options)
