...
Deep dive into fairlead's permission system. Shows exact match rules, wildcard patterns, operation defaults, global defaults, `PermissionDeniedError`, and the `on_permission_request` callback for interactive approval.

## What You'll Learn

- Permission resolution order (exact > wildcard > operation default > global default)
- Setting per-operation and wildcard permissions in `FairleadOptions`
...
4. **Global default**: `FairleadOptions(default_permission="ask")` — fallback for everything else
