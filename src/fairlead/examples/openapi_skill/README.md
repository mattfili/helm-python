...
Turn any OpenAPI 3.x spec into a fairlead skill with the `openapi()` factory. Load from a URL or local file, discover endpoints via `search()`, and call them with typed results.
...
- Calling API endpoints through fairlead's typed interface
